...
- 用户直属上级属于部门维度组织关系，数据落在 `sys_user_depart.supervisor_user_id`，用户详情可展示摘要，具体维护入口优先放在部门成员关系配置中。
...
- 审批中心需要的主部门、部门负责人和直属上级关系不写入 `sys_user` 主账号表，统一复用 `sys_user_depart`，避免同一用户跨租户或跨部门时组织关系混淆。
