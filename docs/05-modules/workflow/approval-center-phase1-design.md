...
以下能力不进入一期实现，并按后续交付节奏拆分为二期和三期：

- 二期聚焦运行时增强和常用协同能力，范围以 [审批中心二期设计文档](approval-center-phase2-design.md) 为准。
- 三期聚焦流程治理、模板复用、打印下载和归档留存能力，范围以 [审批中心三期规划文档](approval-center-phase3-design.md) 为准；审批统计和报表、监控大屏、移动端专项适配、第三方系统开放接口统一放入后续规划。
...
- `assignee_type=starter_select` 时，流程设计阶段不配置可选范围；运行到上一环节办理人点击通过时，从本租户有效用户中选择该节点审批人，选择结果写入实例审批人快照。界面文案为“审批人自选”。
...
任务候选人表。用于指定角色、部门负责人、部门岗位等解析出多人且明确启用候选池策略时，支撑“我的待办”列表和认领/处理权限判断。
...
| 审批详情/办理 | 列表内统一抽屉打开 | 发起申请、我的待办、我的已办、我发起的统一复用运行时表单抽屉，展示表单、审批意见、抄送信息和办理操作 |
...
- `POST /workflow/instance/detail` 返回流程实例扩展信息、表单实例快照、当前待办任务、审批记录和二期抄送记录。
...
- 发起人直属上级在运行时解析为空时，不直接阻断流转，统一按“审批人自选”兜底进入人员选择器；不能用部门负责人或其他组织关系替代。
- 流程运行中任何支持的审批人类型解析下一节点审批人为空时，不直接阻断流转；系统提示当前办理人，并以“审批人自选”兜底选择本租户有效用户，选择结果写入 `wf_process_instance_assignee` 后继续流转，避免生成悬空待办。
...
以下内容不进入一期，后续不再统一称为“二期大包”，而是拆成二期和三期分别设计、开发和验收。
