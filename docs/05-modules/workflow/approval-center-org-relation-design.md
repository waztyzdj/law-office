...
- 指定人员、指定角色可以直接基于现有用户和角色体系实现；其中指定人员若配置多个有效用户，运行时与指定角色、部门岗位一样，由上一步办理人选择具体执行人。
...
- 流程运行中任何支持的审批人类型解析下一节点审批人为空时，提示当前办理人并以“审批人自选”兜底选择本租户有效用户，不自动跳过节点。
...
{
  "departSource": "starter",
  "emptyStrategy": "starter_select_fallback"
}
```
...
- `emptyStrategy=starter_select_fallback`：找不到直属上级或直属上级不是当前租户有效用户时，不直接阻断流程；提示当前办理人，并按“审批人自选”兜底选择本租户有效用户。
...
- 发布前能确定配置为空的固定配置仍应阻断发布，并提示具体节点。
- 流程运行中任何支持的审批人类型解析下一审批节点为空时，不自动跳过节点；提示当前办理人，并按“审批人自选”兜底选择本租户有效用户后继续流转。
- 发起人直属上级未维护、直属上级为空或直属上级失效时，按运行时空审批人策略进入“审批人自选”兜底，不能用部门负责人替代。
